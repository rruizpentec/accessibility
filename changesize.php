...
// If the user hasn't already changed the size, we need to find a default/referent so we know where we're inc/decreasing from.
...
if (!accessibility_is_ajax()) {
    // Redirect the user. If action is not achieved through ajax, redirect back to page is required.
    $redirect = required_param('redirect', PARAM_TEXT);
