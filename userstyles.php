...
$blockinstance = block_instance('accessibility', $data); // Test it in all languages?
// If (!$blockinstance) error...
// READ USER SETTINGS.
...
for ($i = 2; $i < 5; $i++) {  // This is how many declarations we defined in defaults.php.
    ...
    if (!empty($fg)) {
        echo 'color:'.$fg.' !important;';
    }
    if (!empty($bg)) {
        echo 'background-color:'.$bg.' !important;';
    }
    echo '}';
}

// Display:inline-block CSS declaration is not applied to block's buttons because IE7 doesn't support it.
